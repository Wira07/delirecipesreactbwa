...
export default function SearchDetails() {
  ...
  const performSearch = async (query: string) => {
    ...
    try {
      const response = await axios.get(`http://delirecipesbwa.test/api/recipes/search?query=${query}`);
  ...
  return (
    <>
      ...
      <section id="SearchResult" className="px-5 mt-[30px]">
        ...
        <div className="flex flex-col gap-[18px] mt-[18px]">{searchResults.length > 0 ? searchResults.map((recipe) => <RecipeCardResult key={recipe.id} recipe={recipe}></RecipeCardResult>) : <p>Belum ada recipe terkait</p>}</div>
      </section>
    </>
  );
}
